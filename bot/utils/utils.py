...
def get_bot_photo_id() -> FSInputFile:
    return FSInputFile("bot/static/no_image.jpg")
...
def get_text(language: Language, entity: BotEntity, key: str) -> str:
    try:
        with open(f"bot/i18n/{language.value}.json", "r", encoding="utf-8") as f:
            data = json.load(f)

        return data[entity.name.lower()][key]

    except Exception:
        if language == Language.EN:
            return f"{entity.name}.{key}"
