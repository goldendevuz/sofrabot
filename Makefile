...
run-asgi:
	uvicorn core.asgi:application --host 0.0.0.0 --port 2026 --reload
run:
...
run-wsgi:
	gunicorn core.wsgi:application --bind 0.0.0.0:2026
tunnel:
	jprq http 2026 -s vita-api
open-bash:
	sudo docker exec -it vita_api bash
down:
...
restart:
	sudo docker rm -f vita_api vita_nginx vita_redis & make down & make up
seed_languages:
